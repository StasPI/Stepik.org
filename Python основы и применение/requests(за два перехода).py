...
count = 0
first_res = requests.get(start_url[0])
first_list_url = re.findall(
    r'<a href="(https://.+?\.html)">\d</a>', str(first_res.content))
for element_url in first_list_url:
    second_res = requests.get(element_url)
    second_list_url = re.findall(
        r'<a href="(https://.+?\.html)">\d</a>', str(second_res.content))
    if start_url[1] in second_list_url:
        count += 1
if count >= 1:
    print('Yes')
else:

...
последовательности a, что f(x) равно True. Будем говорить, что 
в этом случае функция f допускает элемент x, а элемент x является допущенным.
В данной задаче мы просим вас реализовать класс multifilter, который 
будет выполнять ту же функцию, что и стандартный класс filter, но будет 
...
В этом примере мы хотим отфильтровать последовательность a и оставить только те элементы, которые
...
judge_any(0, 2) равно False, значит мы не допускаем элемент x = 1. Возьмем второй элемент x = 2.
f2(x) равно True
f3(x) равно False
pos = 1, neg = 1
judge_any(1, 1) равно True, значит мы допускаем элемент x = 2. Аналогично для третьего элемента x = 3.
...
# 1 __init__ записываем только сами переменные;
#     2 __iter__ у нас генератор! пробегаемся по нашему списку self.iterable, внутри цикла for первым
# делом создаем переменные pos, neg=0, 0
#     2.1 * funcs - представляет собой кортеж функций, пробегаемся по ним и выполняем для полученного
# элемента списка. От результата каждой функции(true, false)
# зависит какую из переменных pos и neg мы увеличиваем.
#     2.2 После того как мы пробежались по функциям * funcs - проверяем значение по контрольной функции
# и если оно true, то (yield) значение из списка.

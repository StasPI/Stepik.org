...
try:
   ...
   print("<имя 2>")
...
Костя посмотрел на этот код и указал Антону на то, что некоторые исключения можно не ловить, так как ранее 
в коде будет пойман их предок. Но Антон не помнит какие исключения наследуются от каких. Помогите ему выйти
 из неловкого положения и напишите программу, которая будет определять обработку каких исключений можно 
 удалить из кода.
Важное примечание:
...
def search(scopes, heir):
    if heir in scopes:
        for father in scopes[heir]['parent']:
            if father in stek_set:
                error_set.add(element)
                return element
            elif element in error_set:
                return element
            else:
                try:
                    search(scopes, father)
                except:
                    return element
...
for element in range(search_str):
    element = input()
    stek_set.add(element)
    if element not in error_set:
        if search(scopes, element) is not None or element in error_set:
            print(search(scopes, element))